...
# ========== 1. INLINE-МЕНЮ (працює в чатах через @) ==========
...
def inline_handler(inline_query):
    ...
    for amt in amounts:
        ...
        # ТУТ ТІЛЬКИ URL (посилання на бота), БЕЗ WEBAPP
        keyboard.add(
            InlineKeyboardButton(
                f"🤖 Забрать {amt} звёзд",
                url="https://t.me/StarCheclBot"  # Просто посилання на бота
            )
        )
        results.append(
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    ...
                    f"👇 Нажми на кнопку, чтобы перейти в бота.",
                    parse_mode='Markdown'
                ),
...
# ========== 2. ОСОБИСТИЙ ЧАТ (кнопка з WebApp) ==========
...
def start_cmd(message):
    ...
    keyboard.add(
        InlineKeyboardButton(
            ...
            web_app=WebAppInfo(url=WEBAPP_URL)  # ТУТ WEBAPP
        )
    )
    bot.send_message(
        message.chat.id,
        ...
        "👇 Нажми на кнопку ниже, чтобы открыть мини-приложение и ввести свои данные.\n\n"
